...
function AnalyticsDashboard({
  ...
  const traffic = useMemo(() => {
    ...
    const pageEvents = events.filter((event) => event.type === 'page_view' && event.timestamp >= cutoff);
    ...
    return {
      ...
      sessions: period === 'all'
        ...
        : events.filter((event) => event.type === 'session_start' && event.timestamp >= cutoff).length,
      pages: sortedPages,
  ...
  const dcList = Array.isArray(discussionComments) ? discussionComments : [];

  const answeredQuestions = qList.filter((question) => aList.some((answer) => answer.questionId === question.id)).length;
  const acceptedQuestions = qList.filter((question) => question.acceptedAnswerId > 0).length;
  const verifiedProviders = uList.filter((user) => user.role === 'Service Provider' && user.verified).length;
  const serviceProviders = uList.filter((user) => user.role === 'Service Provider').length;
  const approvedListings = lList.filter((listing) => listing.isApproved).length;
  const totalListingValue = lList
    .filter((listing) => listing.isApproved)
    .reduce((sum, listing) => sum + listing.price, 0);
  const interactionCount = aList.length + cList.length + dcList.length + mList.length + rList.length;
  const maxPageVisits = traffic.pages[0]?.[1] ?? 1;

  const roleBreakdown = [
    { label: 'Car owners', value: uList.filter((user) => user.role === 'Car Owner').length, color: colorsForAnalytics.primary },
    { label: 'Service providers', value: serviceProviders, color: colorsForAnalytics.proCircle },
    { label: 'Admins', value: uList.filter((user) => user.role === 'Admin').length, color: colorsForAnalytics.destructive },
  ];

  const contributorRows = uList
    .map((user) => ({
      name: user.name,
      score:
        qList.filter((question) => question.userId === user.id).length * 3 +
        aList.filter((answer) => answer.userId === user.id).length * 2 +
        lList.filter((listing) => listing.userId === user.id).length,
    }))
