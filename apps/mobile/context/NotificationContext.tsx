...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setPreferences(preferenceResponse?.preferences ?? null);
      setGlobalSettings(Array.isArray(preferenceResponse?.globalSettings) ? preferenceResponse.globalSettings : []);
  ...
  const markRead = useCallback(async (id: number) => {
    setNotifications((current) => (Array.isArray(current) ? current.map((item) => (item.id === id ? { ...item, isRead: true } : item)) : []));
    try {
  ...
  const markAllRead = useCallback(async () => {
    setNotifications((current) => (Array.isArray(current) ? current.map((item) => ({ ...item, isRead: true })) : []));
    try {
