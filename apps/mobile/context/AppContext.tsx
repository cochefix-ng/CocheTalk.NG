...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncBackend = useCallback(async () => {
    try {
      ...
      const questions = Array.isArray(bootstrap.questions) ? (bootstrap.questions as Question[]) : null;
      const discussions = Array.isArray(bootstrap.discussions) ? (bootstrap.discussions as DiscussionPost[]) : null;
      const listings = Array.isArray(bootstrap.listings) ? (bootstrap.listings as MarketplaceListing[]) : null;
      const answersArr = Array.isArray(bootstrap.answers) ? (bootstrap.answers as Answer[]) : null;
      const commentsArr = Array.isArray(bootstrap.comments) ? (bootstrap.comments as Comment[]) : null;
      const discussionCommentsArr = Array.isArray(bootstrap.discussionComments) ? (bootstrap.discussionComments as DiscussionComment[]) : null;
      ...
      setState(prev => {
        ...
        return {
          ...
          answers: answersArr ?? prev.answers ?? [],
          comments: commentsArr ?? prev.comments ?? [],
          discussionComments: discussionCommentsArr ?? prev.discussionComments ?? [],
        };
      });
    } catch (e) {
    ...
    async (id: number) => {
      ...
      save({
        ...
        questions: state.questions.map((q) => {
          ...
          return {
            ...q,
            upvotes: isUpvoted ? Math.max(0, (q.upvotes || 0) - 1) : (q.upvotes || 0) + 1,
            upvotedBy: isUpvoted
              ? upvotedBy.filter((uid) => uid !== currentUser.id)
    ...
    async (id: number) => {
      ...
      save({
        ...
        answers: state.answers.map((a) => {
          if (a.id !== id) return a;
          const upvotedBy = Array.isArray(a.upvotedBy) ? a.upvotedBy : [];
          const isUpvoted = upvotedBy.includes(currentUser.id);
          return {
            ...a,
            upvotes: isUpvoted ? Math.max(0, (a.upvotes || 0) - 1) : (a.upvotes || 0) + 1,
            upvotedBy: isUpvoted
              ? upvotedBy.filter((uid) => uid !== currentUser.id)
              : [...upvotedBy, currentUser.id],
          };
    ...
    async (id: number) => {
      ...
      save({
        ...
        discussions: state.discussions.map((d) => {
          if (d.id !== id) return d;
          const upvotedBy = Array.isArray(d.upvotedBy) ? d.upvotedBy : [];
          const isUpvoted = upvotedBy.includes(currentUser.id);
          return {
            ...d,
            upvotes: isUpvoted ? Math.max(0, (d.upvotes || 0) - 1) : (d.upvotes || 0) + 1,
            upvotedBy: isUpvoted
              ? upvotedBy.filter((uid) => uid !== currentUser.id)
              : [...upvotedBy, currentUser.id],
          };
  ...
  const unreadCount = useMemo(() => {
    ...
    return (state.conversations ?? []).filter((c) => Array.isArray(c?.unreadBy) && c.unreadBy.includes(currentUser.id)).length;
  }, [state, currentUser]);
  ...
  const value = useMemo<AppContextType>(() => {
    ...
    return {
      ...current,
      users: Array.isArray(current.users) ? current.users : seed.users,
      ...
      discussions: Array.isArray(current.discussions) ? current.discussions : [],
      discussionComments: Array.isArray(current.discussionComments) ? current.discussionComments : [],
