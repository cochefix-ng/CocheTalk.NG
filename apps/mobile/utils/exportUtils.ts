...
export async function exportUsersReport(
  ...
  const lList = Array.isArray(listings) ? listings : [];
  const rList = Array.isArray(ratings) ? ratings : [];

  const data = uList.map((u) => {
    const userQs = qList.filter((q) => q.userId === u.id).length;
    const userAs = aList.filter((a) => a.userId === u.id).length;
    const userLs = lList.filter((l) => l.userId === u.id).length;
    const userRatings = rList.filter((r) => r.providerId === u.id);
    const avgR =
      userRatings.length
...
export async function exportAnalyticsReport(
  ...
  const carOwners = uList.filter((u) => u.role === 'Car Owner').length;
  const serviceProviders = uList.filter((u) => u.role === 'Service Provider').length;
  const verifiedProviders = uList.filter((u) => u.role === 'Service Provider' && u.verified).length;
  const bannedUsers = uList.filter((u) => u.isBanned).length;

  const publicQs = qList.filter((q) => !q.isPrivateEcosystem).length;
  const proQs = qList.filter((q) => q.isPrivateEcosystem).length;
  const answeredQs = qList.filter((q) => aList.some((a) => a.questionId === q.id)).length;
  const acceptedQs = qList.filter((q) => q.acceptedAnswerId).length;
  const avgAnswers = qList.length
    ? (aList.length / qList.length).toFixed(2)
    : '0';

  const approvedListings = lList.filter((l) => l.isApproved).length;
  const pendingListings = lList.filter((l) => !l.isApproved).length;
  const featuredListings = lList.filter((l) => l.isFeaturedBottom).length;
  const partsListings = lList.filter((l) => l.category === 'Parts').length;
  const servicesListings = lList.filter((l) => l.category === 'Services').length;
  const carSalesListings = lList.filter((l) => l.category === 'Car Sales').length;
  const totalListingValue = lList
